<?php
namespace AmaTests\Entity\Repository;
...
class TestRepository extends EntityRepository
{
    ...
    public function findTests($user, $categories = '', $categoryId='', $args = array())
    {
        $catArray = array();
        ...
        $query->select($select)
            ...
            ->join('AmaTests\Entity\Test', 't', 'WITH', 't.id=f.test')
        ...
        if($categoryId)
            $getQuery->useResultCache($this->cache, $this->cacheTime,  'AmaTests' .$categoryId . $user->getId() );
        else
            $getQuery->useResultCache($this->cache, $this->cacheTime,  'AmaTests' . $user->getId() );
    ...
    public function findTestsCount($user, $categories = '', $categoryId='', $args = array())
    {
        $catArray = array();
        if (!is_array($categories) ) $catArray[] = $categories;
        else $catArray = $categories;
        ...
        if($categoryId)
            $getQuery->useResultCache($this->cache, $this->cacheTime,  'AmaTestsCount' .$categoryId . $user->getId() );
        else
            $getQuery->useResultCache($this->cache, $this->cacheTime,  'AmaTestsCount' . $user->getId() );
    ...
    public function checkIfUserCanAccessTest($user, $id, $asLpf = true)
    {
        $query = $this->getEntityManager()->createQueryBuilder();
        $select = array('f');
        $query->select($select)
        ...
        //$getQuery->useResultCache($this->cache, $this->cacheTime,  'AmaSolveTest' .  $id . $user->getId());
    ...
    public function findTestById($id, $asLpf = true, $getPoints = false)
    {

        $query1= $this->getEntityManager()->createQueryBuilder();
        $subQ1 = $query1
            ->select('SUM(q2.points)')
            ->from('AmaTests\Entity\Question', 'q2')
            ->where('q2.test = f.test');

        $query2 = $this->getEntityManager()->createQueryBuilder();
        $subQ2 = $query2
            ->select('SUM(utaq2.points)')
            ->from('AmaTests\Entity\UserTestAnswer', 'utaq2')
            ->where('utaq2.test = f.test')
        ...
        $getQuery->useResultCache($this->cache, $this->cacheTime,  'AmaSolveTest' .  $id);
    ...
    public function findQuestionsByTestId($id)
    {

        $query = $this->getEntityManager()->createQueryBuilder();
        $select = array('q');
        $query->select($select)
            ->from('AmaTests\Entity\Question', 'q')
            ->where('q.test=:test')
        ...
        $getQuery->useResultCache($this->cache, $this->cacheTime,  'AmaSolveTestQuestions' .  $id);
    ...
    public function findAnswersByQuestionId($id, $testId)
    {
        $query = $this->getEntityManager()->createQueryBuilder();
        $select = array('a');
        $query->select($select)
            ->from('AmaTests\Entity\Answer', 'a')
            ->where('a.question=:q')
        ...
        $getQuery->useResultCache($this->cache, $this->cacheTime,  'AmaSolveTestAnswers' . $id . $testId);
    ...
    public function findRightAnswersByQuestionId($id, $testId)
    {
        $query = $this->getEntityManager()->createQueryBuilder();
        $query->select('a.id')
            ->from('AmaTests\Entity\Answer', 'a')
        ...
        $getQuery->useResultCache($this->cache, $this->cacheTime,  'AmaSolveTestRightAnswers' . $id . $testId);
    ...
    public function findImagesByQuestionId($id, $testId)
    {
        $query = $this->getEntityManager()->createQueryBuilder();
        $select = array('qi');
        $query->select($select)
            ->from('AmaTests\Entity\QuestionImage', 'qi')
            ->where('qi.question=:q')
            ->setParameter("q", $id);

        $getQuery = $query->getQuery();

        $getQuery->useResultCache($this->cache, $this->cacheTime,  'AmaSolveTestImages' . $id .  $testId);
    ...
    public function findTestStatistics($lessonPlanId, $fileClassId, $userId)
    {
        ...
        $subQ3 = $query3
            ->select('SUM(utaq2.points)')
            ->from('AmaTests\Entity\UserTestAnsweredQuestion', 'utaq2')
            ->where('utaq2.test = f.test')
            ->andWhere('utaq2.lessonPlan = lpf.id')
            ->andWhere('utaq2.fileClass = fc.id')
        ...
        $getQuery = $query->getQuery();
        $getQuery->useResultCache($this->cache, $this->cacheTime,  'AmaTestStatistics' . $lessonPlanId . $fileClassId .  $userId);
    ...
    public function findTestStatisticsQuestions($test, $lessonPlanId, $fileClassId, $user)
    {

        $query1 = $this->getEntityManager()->createQueryBuilder();
        $subQ = $query1
            ->select('COUNT(utaq1.id)')
            ->from('AmaTests\Entity\UserTestAnswer', 'utaq1')
            ->where('utaq1.test = f.test')
        ...
        $query->select($select)
            ...
            ->from('AmaTests\Entity\Question', 'q')
            ->join('AmaTests\Entity\Test', 't', 'WITH', 't.id=q.test')
            ->join('AmaMaterials\Entity\File', 'f', 'WITH', 'f.test=t.id')
        ...
        $getQuery->useResultCache($this->cache, $this->cacheTime,  'AmaTestStatisticsQuestions' . $lessonPlanId . $fileClassId . $user->getId());
    ...
    public function findTestStatisticsQuestionAnswers($lessonPlanId, $fileClassId, $user, $question)
    {
        $query = $this->getEntityManager()->createQueryBuilder();
        $select = array('a', 's', 'ua', 'utaq', 'u');
        $query->select($select)
            ->from('AmaTests\Entity\UserTestAnswer', 'ua')
            ->join('AmaTests\Entity\UserTestAnsweredQuestion', 'utaq', 'WITH', 'utaq.question=ua.question')
            ->leftJoin('Amatests\Entity\Answer', 'a' , 'WITH', 'a.id=ua.answer')
        ...
        $getQuery->useResultCache($this->cache, $this->cacheTime,  'AmaTestStatisticsQuestionAnswers' . $lessonPlanId . $fileClassId . $question . $user);
